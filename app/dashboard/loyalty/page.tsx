  ...
  const Topbar = (
    <header style={{ background:"#1C2438", borderBottom:"1px solid #2a3350", padding:"0 24px", height:66, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:30, boxShadow:"0 1px 3px rgba(0,0,0,0.04)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:14 }}>
        ...
        <div>
          <div style={{ fontSize:15, fontWeight:800, color:"#F7F5EF" }}>🏆 Loyalty Points</div>
          <div style={{ fontSize:11.5, color:"#94A3B8", marginTop:1 }}>Reward your loyal clients</div>
        </div>
      </div>
      <div style={{ display:"flex", gap:8 }}>
        <button onClick={() => setShowSetup(true)} style={{ padding:"9px 14px", background:"#141A2E", border:"1.5px solid #2a3350", borderRadius:12, fontSize:13, fontWeight:700, color:"#475569", cursor:"pointer" }}>⚙️ Settings</button>
    ...
    <DashboardShell salonName={salonName} topbar={Topbar}>
      <div style={{ padding:"28px 24px", maxWidth:1360, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:24 }}>
          {tiers.map((tier: Tier) => (
            <div key={tier.name} style={{ background:"#1C2438", border:`1.5px solid ${tier.color}30`, borderRadius:16, padding:"16px", borderTop:`3px solid ${tier.color}` }}>
        ...
        <div style={{ background:"#1C2438", border:"1.5px solid #2a3350", borderRadius:20, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.03)" }}>
          <div style={{ padding:"16px 22px", borderBottom:"1px solid #2a3350" }}>
            <div style={{ fontSize:15, fontWeight:800, color:"#F7F5EF" }}>All Members <span style={{ fontSize:12, color:"#94A3B8", fontWeight:600 }}>({filtered.length})</span></div>
          </div>
          {filtered.length === 0 ? (
                ...
                <thead>
                  <tr style={{ background:"#141A2E" }}>
                    {["Client","Tier","Points","Earned","Redeemed","Actions"].map(h => (
                      <th key={h} style={{ fontSize:10, fontWeight:900, color:"#94A3B8", textAlign:"left", padding:"11px 16px", letterSpacing:"0.8px", textTransform:"uppercase", borderBottom:"1px solid #2a3350" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={c.id} style={{ transition:"background 0.1s" }}
                        ...
                        <td style={{ padding:"12px 16px", borderBottom:"1px solid #2a3350" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                            ...
                            <div>
                              <div style={{ fontSize:13.5, fontWeight:800, color:"#F7F5EF" }}>{c.client_name}</div>
                        ...
                        <td style={{ padding:"12px 16px", borderBottom:"1px solid #2a3350" }}>
                          ...
                          {next && (
                            <div style={{ marginTop:6 }}>
                              <div style={{ height:4, background:"#2a3350", borderRadius:99, width:80 }}>
                        ...
                        <td style={{ padding:"12px 16px", borderBottom:"1px solid #2a3350", fontSize:18, fontWeight:900, color:tier.color }}>{c.points.toLocaleString()}</td>
                        <td style={{ padding:"12px 16px", borderBottom:"1px solid #2a3350", fontSize:13, color:"#10B981", fontWeight:700 }}>+{c.total_earned}</td>
                        <td style={{ padding:"12px 16px", borderBottom:"1px solid #2a3350", fontSize:13, color:"#EF4444", fontWeight:700 }}>-{c.total_redeemed}</td>
                        <td style={{ padding:"12px 16px", borderBottom:"1px solid #2a3350" }}>
                          <button onClick={() => { setSelectedClient(c); setShowModal(true); }} style={{ padding:"6px 14px", background:"linear-gradient(135deg,#C9A24B,#0E1320)", color:"#fff", border:"none", borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer" }}>Adjust</button>
                        </td>
                      </tr>
                    );
                  })}
      ...
      {showModal && selectedClient && (
        <div onClick={() => setShowModal(false)} style={{ position:"fixed", inset:0, background:"rgba(15,23,42,0.55)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:16, backdropFilter:"blur(4px)" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#1C2438", borderRadius:20, padding:28, width:"100%", maxWidth:420, boxShadow:"0 32px 80px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize:18, fontWeight:900, color:"#F7F5EF", marginBottom:4 }}>Adjust Points</div>
            <div style={{ fontSize:13, color:"#64748B", marginBottom:20 }}>{selectedClient.client_name} · <strong style={{ color:getTier(selectedClient.points, tiers).color }}>{selectedClient.points} pts</strong></div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
                  {(["earn","redeem","bonus"] as const).map(t => (
                    <button key={t} onClick={() => setAdjForm({...adjForm, type:t})}
                      style={{ padding:"8px 4px", borderRadius:10, border:`1.5px solid ${adjForm.type===t ? "#C9A24B" : "#2a3350"}`, background: adjForm.type===t ? "rgba(201,162,75,0.10)" : "#1C2438", color: adjForm.type===t ? "#C9A24B" : "#aab1c4", fontSize:12.5, fontWeight:700, cursor:"pointer", textTransform:"capitalize", transition:"all 0.12s" }}>
                      {t === "earn" ? "💰" : t === "redeem" ? "🎁" : "⭐"} {t}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ display:"flex", gap:10, marginTop:20 }}>
              ...
              <button onClick={handleAdjust} disabled={saving || !adjForm.points} style={{ flex:2, padding:12, background:"linear-gradient(135deg,#C9A24B,#0E1320)", border:"none", borderRadius:12, fontSize:13.5, fontWeight:700, color:"#fff", cursor:"pointer", opacity:!adjForm.points?0.5:1 }}>
      ...
      {showSetup && (
        <div onClick={() => setShowSetup(false)} style={{ position:"fixed", inset:0, background:"rgba(15,23,42,0.55)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:16, backdropFilter:"blur(4px)" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#1C2438", borderRadius:20, padding:28, width:"100%", maxWidth:420, boxShadow:"0 32px 80px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize:18, fontWeight:900, color:"#F7F5EF", marginBottom:20 }}>⚙️ Loyalty Settings</div>
            <div>
            ...
            <div style={{ marginTop:16, padding:"14px 16px", background:"rgba(201,162,75,0.10)", borderRadius:12 }}>
              <div style={{ fontSize:12.5, fontWeight:700, color:"#C9A24B", marginBottom:8 }}>Tier Breakdown</div>
              {tiers.map((t: Tier) => (
            ...
            <button onClick={() => { toast.success("Settings saved!"); setShowSetup(false); }} style={{ marginTop:20, width:"100%", padding:12, background:"linear-gradient(135deg,#C9A24B,#0E1320)", border:"none", borderRadius:12, fontSize:13.5, fontWeight:700, color:"#fff", cursor:"pointer" }}>Save Settings</button>
          </div>
        </div>
      )}
    </DashboardShell>
