...
const STATUS_MAP = {
  ...
  contacted: { label: "Contacted", bg: "rgba(201,162,75,0.10)", color: "#C9A24B" },
  booked:    { label: "Booked ✓",  bg: "rgba(16,185,129,0.10)", color: "#10B981" },
  removed:   { label: "Removed",   bg: "#2a3350", color: "#aab1c4" },
};
  ...
  const Topbar = (
    <header style={{ background: "#1C2438", borderBottom: "1px solid #2a3350", padding: "0 24px", height: 66, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 30, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: "#F7F5EF" }}>⏳ Waitlist</div>
          <div style={{ fontSize: 11.5, color: "#aab1c4", marginTop: 1 }}>Manage clients waiting for slots</div>
        </div>
      </div>
    ...
    <DashboardShell salonName={salonName} topbar={Topbar}>
      <div style={{ padding: "28px 24px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
          {([["all","Total","⏳","#C9A24B"], ["waiting","Waiting","🕐","#F59E0B"], ["contacted","Contacted","📞","#C9A24B"], ["booked","Booked","✅","#10B981"]] as const).map(([key, label, icon, color]) => (
            <div key={key} onClick={() => setFilter(key as typeof filter)} style={{ background: filter === key ? "rgba(255,255,255,0.06)" : "#1C2438", border: `2px solid ${filter === key ? color : "#2a3350"}`, borderRadius: 16, padding: "18px 16px", cursor: "pointer", position: "relative", overflow: "hidden", transition: "all 0.15s" }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 900, color: "#F7F5EF" }}>{key === "all" ? entries.length : entries.filter(e => e.status === key).length}</div>
            </div>
          ))}
        </div>
                  ...
                  {filtered.map((e, i) => {
                    ...
                    return (
                      <tr key={e.id} style={{ transition: "background 0.1s" }}
                        onMouseEnter={ev => { (ev.currentTarget as HTMLTableRowElement).style.background = "#141A2E"; }}
                        onMouseLeave={ev => { (ev.currentTarget as HTMLTableRowElement).style.background = "transparent"; }}>
                        <td style={{ padding: "12px 16px", borderBottom: "1px solid #2a3350", fontSize: 13, color: "#aab1c4", fontWeight: 700 }}>{i + 1}</td>
                        <td style={{ padding: "12px 16px", borderBottom: "1px solid #2a3350" }}>
                          <div style={{ fontSize: 13.5, fontWeight: 800, color: "#F7F5EF" }}>{e.client_name}</div>
                          <div style={{ fontSize: 11.5, color: "#aab1c4" }}>{e.client_phone || e.client_email || "—"}</div>
                        </td>
                        ...
                        <td style={{ padding: "12px 16px", borderBottom: "1px solid #2a3350", fontSize: 12.5, color: "#aab1c4", maxWidth: 200 }}>{e.notes || "—"}</td>
                        <td style={{ padding: "12px 16px", borderBottom: "1px solid #2a3350" }}>
                          <select value={e.status} onChange={ev => updateStatus(e.id, ev.target.value as WaitlistEntry["status"])}
                            style={{ padding: "5px 10px", borderRadius: 8, border: "1.5px solid #2a3350", fontSize: 12, fontWeight: 700, cursor: "pointer", background: sm.bg, color: sm.color, outline: "none" }}>
                          ...
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            ...
                            {e.status === "waiting" && (e.client_phone || e.client_email) && (
                              <button
                                ...
                                style={{ padding: "5px 10px", background: notifying === e.id ? "#2a3350" : "linear-gradient(135deg,#C9A24B,#C2185B)", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: notifying === e.id ? "not-allowed" : "pointer", color: notifying === e.id ? "#94A3B8" : "#1C2438", whiteSpace: "nowrap" }}
                              >
                                {notifying === e.id ? "Sending…" : "🔔 Notify"}
                              </button>
                            )}
                            {e.client_phone && <button onClick={() => { navigator.clipboard.writeText(e.client_phone); toast.success("Phone copied!"); }} style={{ padding: "5px 10px", background: "#141A2E", border: "1.5px solid #BBF7D0", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#10B981" }}>📞</button>}
                            <button onClick={() => deleteEntry(e.id)} style={{ padding: "5px 10px", background: "#141A2E", border: "1.5px solid #FECACA", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#DC2626" }}>✕</button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.55)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, backdropFilter: "blur(4px)" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#1C2438", borderRadius: 20, padding: 28, width: "100%", maxWidth: 460, boxShadow: "0 32px 80px rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize: 18, fontWeight: 900, color: "#F7F5EF", marginBottom: 20 }}>⏳ Add to Waitlist</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                <div key={f.key}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#aab1c4", display: "block", marginBottom: 6 }}>{f.label}</label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#aab1c4", display: "block", marginBottom: 6 }}>Preferred Date</label>
                  <input type="date" value={form.preferred_date} onChange={e => setForm(p => ({ ...p, preferred_date: e.target.value }))} style={{ width: "100%", padding: "10px 13px", border: "1.5px solid #2a3350", borderRadius: 10, fontSize: 14, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: "#aab1c4", display: "block", marginBottom: 6 }}>Preferred Time</label>
                  <input type="time" value={form.preferred_time} onChange={e => setForm(p => ({ ...p, preferred_time: e.target.value }))} style={{ width: "100%", padding: "10px 13px", border: "1.5px solid #2a3350", borderRadius: 10, fontSize: 14, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#aab1c4", display: "block", marginBottom: 6 }}>Notes</label>
                <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Service requested, special requirements…" rows={2} style={{ width: "100%", padding: "10px 13px", border: "1.5px solid #2a3350", borderRadius: 10, fontSize: 14, fontFamily: "inherit", outline: "none", resize: "none", boxSizing: "border-box" }} />
              </div>
            </div>
