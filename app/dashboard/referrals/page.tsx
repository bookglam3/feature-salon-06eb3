    ...
    <DashboardShell salonName={salonName} topbar={Topbar}>
      <div style={{ padding: "28px 24px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
          {[
            { label: "Total Referrals", value: referrals.length, color: "#6366F1" },
                  ...
                  {referrals.map(r => {
                    ...
                    return (
                      <tr key={r.id} style={{ transition: "background 0.1s" }}
                        ...
                        <td style={{ padding: "12px 16px", borderBottom: "1px solid #F1F5F9" }}>
                          <button onClick={() => copyLink(r.code)} style={{ padding: "6px 12px", background: "#EEF2FF", color: "#6366F1", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🔗 Copy Link</button>
                        </td>
                      </tr>
                    );
                  })}
