...
/**
 * Route → roles that may access it.
 * Rules are matched longest-prefix-first so more specific routes win.
 * An empty array means "any authenticated admin".
 */
...
function requiredRolesFor(pathname: string): AdminRole[] {
  // Sort longest path first so /admin/users beats /admin
